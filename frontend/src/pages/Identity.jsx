...
import { PageHeader, StatBlock, Card, Reveal, Pill, Department, Expander, EmptyState } from "../ui";
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "44px 24px 64px" }}>
        ...
        <Reveal>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(290px, 1fr))", gap: 20, marginBottom: 40 }}>
            <Card>
              <div style={{ ...TYPE.micro, marginBottom: 12 }}>Audio Profile</div>
            ...
            <Card>
              <div style={{ ...TYPE.micro, marginBottom: 18 }}>Distributions</div>
              <StackBar
                title="Mood"
                parts={[
        ...
        {/* Rabbit holes — tucked away */}
        {rabbit_holes?.length > 0 && (
          <div style={{ marginTop: 40 }}>
            <Expander label="Rabbit Holes" sublabel={`${rabbit_holes.length} artists you binged`}>
              <div style={{ display: "flex", flexDirection: "column" }}>
                {rabbit_holes.map((rh, i) => (
                  <div key={rh.artist} style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 4px", borderTop: i === 0 ? "none" : `1px solid ${C.border}` }}>
                    <span style={{ fontFamily: FONT.display, fontSize: 15, fontWeight: 700, color: C.faint, width: 26, textAlign: "right", flexShrink: 0, fontVariantNumeric: "tabular-nums" }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <div style={{ flex: 1, minWidth: 0 }}>
