...
def get_wrapped(period: Literal["day", "week", "month", "year"] = "month"):
    ...
    # Total minutes
    cur.execute("""
        SELECT COUNT(*) * 3.5 as estimated_minutes
        FROM listening_history
        WHERE played_at >= NOW() - INTERVAL %s
    """, (interval,))
    total_minutes = cur.fetchone()[0] or 0

    # Listening clock — plays by hour of day
    cur.execute("""
...
def sonic_identity():
    """Your personal audio fingerprint from your entire library."""
    conn = get_conn()
    cur = conn.cursor()
    ...
    cur.execute("""
        ...
        FROM tracks WHERE valence IS NOT NULL
    """)
    mood = cur.fetchone()
    ...
    cur.execute("""
        ...
        FROM tracks WHERE energy IS NOT NULL
    """)
    ...
    rabbit_holes = [{
        "artist": r[0],
        "songs_saved": r[1],
        "first_save": str(r[2])[:10],
        "last_save": str(r[3])[:10]
